...
void huh(void) { error("Type H for Help."); }
...
void vksend(const char *fmt, va_list varglist) {
...
void help(char **text, int nlines) {
  ...
  pos_str(text_lines + NUMTOPS + 2, 41,
  ...
  for (i = 0; i < NUM_OBJECTS; i++) {
    ...
    pos_str(r + text_lines + NUMTOPS + 3, c, "%-12s%c     %c%6d%5d%6d",
            piece_attr[i].nickname, piece_attr[i].sname,
            tolower(piece_attr[i].sname), piece_attr[i].speed,
            piece_attr[i].max_hits, piece_attr[i].build_time, 0, 0);  // FLAG
  }
  (void)refresh();
}

#define COL_DIGITS \
  ((MAP_WIDTH <= 100) ? 2 : ((MAP_WIDTH <= 1000 ? 3 : (1 / 0))))
