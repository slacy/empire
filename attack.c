...
void survive(piece_info_t *obj, loc_t loc) {
  while (obj_capacity(obj) < obj->count) kill_obj(obj->cargo, loc);
  ...
  if (win_obj->owner != lose_obj->owner) {
    if (win_obj->owner == USER) {
      ...
      ksend("Enemy %s at %d destroyed.\n", piece_attr[lose_obj->type].name,
            loc_disp(loc));  // kermyt
      topmsg(1, "Enemy %s at %d destroyed.", piece_attr[lose_obj->type].name,
             loc_disp(loc));
      ksend("Your %s has %d hits left\n", piece_attr[win_obj->type].name,
            win_obj->hits);  // kermyt
      topmsg(2, "Your %s has %d hits left.", piece_attr[win_obj->type].name,
             win_obj->hits);
      ...
      if (diff > 0) switch (win_obj->cargo->type) {
          case ARMY:
            ksend("%d armies fell overboard and drowned in the assault.\n",
                  diff);  // kermyt
            topmsg(3, "%d armies fell overboard and drowned in the assault.",
                   diff);
            break;
          case FIGHTER:
